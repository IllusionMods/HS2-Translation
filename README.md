...
English fan translation project for Honey Select 2. The translations are applied when the game is running and do not require replacing or modifying any game files.
...
2. Delete the "**en**" folder in BepInExEx\Translation\\**en** (if it exists)
3. Go to the "releases" page above and download the latest version. Alternatively, advanced users can get the latest beta translations by clicking on the "Clone or download" button above. If you are a translator, read the sections below to see how to contribute to the translations.
4. Extract the zip and place the BepInEx folder inside your game folder (where the file "HoneySelect2.exe" is).
...
Each translation line follows the pattern: `original text=translated text`. Example: `悟飯=Food`.
...
- `Text\Localizations` - Words in this folder have been dumped via TextDump. Translations can be added for missing entries, but new entries should not be added or it will be difficult to merge future dumps. Basically, don't mess with it.
- `Texture` - Contains the translated version of game images. The game images are translated using Photoshop, and the .psd files can be found in the GitHub's folder `Image Source Files`.

Translations will be reviewed before being added via pull-request. So not every translation will automatically go into the database.
...
- If you have more translations to submit [fork the repository](https://help.github.com/articles/fork-a-repo/). This will make a copy of the original project in your account. Upload your changes to the fork into your account, and then [send a pull request](https://help.github.com/articles/about-pull-requests/) to the original project. Your pull request will be reviewed and accepted after a quality check by the team. Again, avoid raw machine translations. Proper capitalization, punctuation, and spelling is a must.
...
Inside each folder there is a file called "translation.txt", this is the file where the translations should go. In some folders there is also the file "zz_machineTranslation.txt" which is where machine translations go, that usually have a lower quality.
...
The file "zz_machineTranslation.txt" can only have the translation lines that are commented out in "translation.txt". If the file "translation.txt" has been fully translated, the file "zz_machineTranslation.txt" should be deleted. The goal of this translation project is that no "zz_machineTranslation.txt" file remains.
...
|----|--------|--------------|--------------------------|---------------------------------------------------------------------------------|
| 0  | クール  | Quiet        | `c00`                      | `hvoicestart_c00_*`, `hvoice_c00_*`, `hvoicestartevXX_c00*`, `hvoiceevXX_c_00*` |
...
| 8  | オープン | Open-hearted | `c08`                      | `hvoicestart_c08_*`, `hvoice_c08_*`, `hvoicestartevXX_c08*`, `hvoiceevXX_c_08*` |
| 9  | 天然   | Natural      | `c09`                      | `hvoicestart_c09_*`, `hvoice_c09_*`, `hvoicestartevXX_c09*`, `hvoiceevXX_c_09*` |
